...
if [ -f "$(dirname "$0")/.env" ]; then
  for VAR in COMPANIES_PER_RUN SYNC_TIME_BUDGET_MS DISCOVERY_PER_RUN DISCOVERY_HOST_CAP DISCOVERY_TIME_BUDGET_MS REQUIREMENTS_PER_RUN REQUIREMENTS_TIME_BUDGET_MS REQUIREMENTS_HOST_CAP REQUIREMENTS_CONCURRENCY DESCRIPTION_STALE_AFTER_DAYS; do
    if [ -z "${!VAR:-}" ]; then
...
for VAR in STRIPE_SECRET_KEY STRIPE_PUBLISHABLE_KEY STRIPE_WEBHOOK_SECRET EARLY_ACCESS_FREE RESEND_API_KEY EMAIL_FROM CRON_SECRET COMPANIES_PER_RUN SYNC_TIME_BUDGET_MS DISCOVERY_PER_RUN DISCOVERY_HOST_CAP DISCOVERY_TIME_BUDGET_MS REQUIREMENTS_PER_RUN REQUIREMENTS_TIME_BUDGET_MS REQUIREMENTS_HOST_CAP REQUIREMENTS_CONCURRENCY DESCRIPTION_STALE_AFTER_DAYS; do
