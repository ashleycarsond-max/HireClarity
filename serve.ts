...
// POST /api/auth/logout, GET /api/auth/me) are served here, outside the
// TanStack router — the installed react-start build has no API-route support.
...
for (let attempt = 1; ; attempt++) {
  ...
  try {
    Bun.serve({
      ...
      hostname: HOST,
      async fetch(req) {
        const cronRes = await handleCronHttp(req);
        if (cronRes) return cronRes;
        const reportRes = await handleReportHttp(req);
        if (reportRes) return reportRes;
        const authRes = await handleAuthHttp(req);
        if (authRes) return authRes;
        const watchRes = await handleWatchHttp(req);
        if (watchRes) return watchRes;
        const stripeRes = await handleStripeHttp(req);
